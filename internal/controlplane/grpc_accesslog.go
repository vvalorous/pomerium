...
func (srv *Server) StreamAccessLogs(stream envoy_service_accesslog_v2.AccessLogService_StreamAccessLogsServer) error {
	for {
		...
		for _, entry := range msg.GetHttpLogs().LogEntry {
			...
			// common properties
			// evt = evt.Str("downstream-local-addr", entry.GetCommonProperties().GetDownstreamLocalAddress().String())
			// evt = evt.Str("downstream-remote-addr", entry.GetCommonProperties().GetDownstreamRemoteAddress().String())
			evt = evt.Str("upstream-cluster", entry.GetCommonProperties().GetUpstreamCluster())
			// evt = evt.Str("upstream-local-addr", entry.GetCommonProperties().GetUpstreamLocalAddress().String())
			// evt = evt.Str("upstream-remote-addr", entry.GetCommonProperties().GetUpstreamRemoteAddress().String())
			// request properties
