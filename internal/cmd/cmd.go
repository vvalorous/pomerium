// Package cmd contains all the pomerium commands and sub-commands.
package cmd
...
// these are single dash flags from before we started using cobra
var legacySingleDashFlags = []string{"version", "config"}

// Execute executes the root command.
func Execute() error {
	args := os.Args[1:]
	for i, arg := range args {
		for _, flag := range legacySingleDashFlags {
...
func init() {
	rootCmd.Flags().BoolVar(&rootOptions.version, "version", false, "print the version")
	rootCmd.Flags().StringVar(&rootOptions.configFilePath, "config", "", "specify configuration file location")
	rootCmd.Args = cobra.ExactArgs(0)
}
...
func rootRun(_ *cobra.Command, _ []string) error {
	...
	opt, err := config.NewOptionsFromConfig(rootOptions.configFilePath)
	if err != nil {
		return err
	}
	...
	opt.OnConfigChange(func(e fsnotify.Event) {
		...
		opt = config.HandleConfigUpdate(rootOptions.configFilePath, opt, []config.OptionsUpdater{authz, proxy})
	})
