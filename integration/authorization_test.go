...
func TestAuthorization(t *testing.T) {
	...
	for _, proxy := range []cluster.Proxy{cluster.ProxyAllInOne, cluster.ProxyInsecure, cluster.ProxySecure} {
		t.Run(proxy.String(), func(t *testing.T) {
			t.Run("public", func(t *testing.T) {
				client := http.DefaultClient

				req, err := http.NewRequestWithContext(ctx, "GET", proxy.DownstreamURL(cluster.EndpointHTTPDetailsTrusted), nil)
				if err != nil {
					t.Fatal(err)
				}
